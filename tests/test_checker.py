...
from ssl_checker.checker import check_ssl, get_ssl_cert
from datetime import datetime, timedelta

@pytest.fixture
def mock_load_domains():
    with patch('ssl_checker.checker.load_domains') as mock:
        mock.return_value = [
...
def mock_get_domain_info():
    with patch('ssl_checker.checker.get_domain_info') as mock:
        mock.return_value = None
        yield mock

@pytest.fixture
def mock_get_ssl_cert():
    with patch('ssl_checker.checker.get_ssl_cert') as mock:
        mock.return_value = {
...
@pytest.fixture
def mock_save_to_db():
    with patch('ssl_checker.checker.save_to_db') as mock:
        yield mock

def test_check_ssl(mock_load_domains, mock_get_domain_info, mock_get_ssl_cert, mock_save_to_db):
    with patch('ssl_checker.checker.datetime') as mock_datetime:
...
def test_get_ssl_cert():
    ...
    with patch('ssl_checker.checker.socket.create_connection') as mock_connection:
        mock_sock = MagicMock()
        mock_connection.return_value = mock_sock

        with patch('ssl_checker.checker.ssl.create_default_context') as mock_context:
